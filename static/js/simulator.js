document.addEventListener('DOMContentLoaded', function() {
  const categories = [
      { key: 'Level', label: '레벨 아이콘' },
      { key: 'Fame', label: '명성 아이콘' },
      { key: 'RP', label: 'RP 아이콘' },
      { key: 'Rate', label: '승패 아이콘' },
      { key: 'Heart', label: '하트' },
      { key: 'Mentor', label: '명소/명테' },
      { key: 'Guild', label: '길드' },
      { key: 'Jewel', label: '보석' },
      { key: 'Frame', label: '테두리' },
      { key: 'NameTag', label: '명패' },
      { key: 'Collection', label: '켜론' },
      { key: 'Skin', label: '대표 스킨' },
      { key: 'RoleBadge', label: '직업 백지' },
      { key: 'Header', label: '지갑 상단' },
      { key: 'ProfileBackground', label: '지갑' }
  ];
  ...
  categories.forEach(({ key, label }) => {
      ...
      button.addEventListener('click', () => loadItems(key));
